...
disp(['--------------------------------------------------------- ']);
    ...
    for Num_Achohor=anchor_min:anchor_gap:anchor_max
        location_error_range_abs =  Num_Achohor;
             disp(['location_error = ',num2str(location_error_range_abs)]);
	disp(['************* ']); 
...
% 
		 ...
		 % temp=randperm(Node_number);		 
		 % Node_Error_NUM=floor(Node_Error_NUM_Percent*Node_number);
		 % err_node=temp(1:Node_Error_NUM);
...
save anchor_Hamming_test.mat  RUNS Size_Grid  anchors   real_statics_run rmse_Hamming_final_Anchors;
save anchor_Advanced_Hamming_test.mat  RUNS Size_Grid  anchors   real_statics_run rmse_Advanced_Hamming_final_Anchors;
save SNR_Weight_Hammingg_test.mat  RUNS Size_Grid  anchors  real_statics_run     rmse_SNR_Weight_Hamming_final_Anchors;
save anchor_Probility_Cutting_test.mat  RUNS Size_Grid  anchors  real_statics_run                   rmse_Probility_Cutting_final_Anchors;




clear all;
load anchor_Hamming_test.mat
load anchor_Advanced_Hamming_test.mat
load anchor_Probility_Cutting_test.mat
...
xlabel('\fontsize{12}\bf Node Location Error');
ylabel('\fontsize{12}\bf Localization Error(in units)');
title('\fontsize{12}\bf  Localization Error vs. Node Location Error');
